...
import 'package:native/repo/model/message.dart' as native;
...
class ChatMessagesCubit extends Cubit<ChatMessagesState> {
  ChatMessagesCubit(this._firestoreRepository)
      : super(ChatMessagesState.initial());

  final FirestoreRepository _firestoreRepository;

  void createChatMessage(String chatRoomDocId, native.Message message) async {
    // emit(const ChatState.loading());

    var response =
        await _firestoreRepository.createChatMessage(chatRoomDocId, message);
  ...
  void getChatMessages(String chatRoomDocId) async {
    ...
    _firestoreRepository.getChatMessages(chatRoomDocId).listen(
      (event) {
        emit(ChatMessagesState.chatMessagesFetched(
            chatMessages: event.map((e) => e.toTextMessage()).toList()));
