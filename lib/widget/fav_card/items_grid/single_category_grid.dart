...
class SingleCategoryGrid extends StatelessWidget {
    ...
    return Column(
      children: [
        Row(children: [
          ...
          Text(
            getCategoryHeading(selectedCategory.name),
  ...
  String getCategoryHeading(FavCardCategoryEnum categoryEnum) {
    if (categoryEnum == FavCardCategoryEnum.popular) {
      return "Popular List";
    } else if (categoryEnum == FavCardCategoryEnum.recommended) {
      return "Recommended List";
    } else {
      return categoryEnum.stringify().capitalize();
    }
  }
}
