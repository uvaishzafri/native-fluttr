...
class AuthState with _$AuthState {
  ...
  const factory AuthState.emailVerificationSent() =
      AuthEmailVerificationSentState;
  const factory AuthState.checkEmailVerification() =
      AuthCheckEmailVerificationState;
  const factory AuthState.emailVerificationComplete() =
      AuthEmailVerificationCompleteState;
  const factory AuthState.errorPincode({required AppException exception}) =
      AuthErrorPincodeState;
  const factory AuthState.failed({required AppException exception}) =
      AuthErrorState;
  const factory AuthState.emailSendFailed({required AppException exception}) =
      AuthEmailSendFailedState;
